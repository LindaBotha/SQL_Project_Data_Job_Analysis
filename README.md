...
<img src="assets/1_demand_all.png" alt="Top data jobs" width="65%">

*Bar chart visualizing the number of job postings per role*     

<img src="assets/1_demand_remote.png" alt="Top remote data jobs" width="65%">
...
From this data, there's a significant number postings for Data Analysts, Data Engineers and Data Scientists, with senior levels of these, as well as other specialized positons having only about ~ 20 % of the available positions compared to the top 3. Since I am new to the world of data, I am not concerned about senior or specialized positions, so I decided to limit my subsequent queries to the top 3 roles ranked by demand. The top 3 roles are also the most abundant in the remote job subset but the breakdown changes slightly, where Data Engineers have significantly more remote job opportunities than Data Scientists or Data Analysts. Remote jobs represent only about 5-10 % of the total available jobs, suggesting a general preference for a physical presence or hybrid work arrangement by companies offering data jobs. 
...
<img src="assets/2_histogram.png" alt="Histogram for skills breakdown" width="65%">
...
### 5. Optimal skills to start learning in the short term
 Building further on the quick insight I gained from just viewing the data in Excel, I wanted to see whether there was a way to combine queries in SQL so I could still focus on high paying skills, however ones that were not as obscure or specialized, in order to equip me for the widest range of potential jobs.
 In order to do this, I combined the sum of the available jobs and average salary aggregations, grouped by skill, though joining  the *skills_dim*, *skills_job_dim* and *job_postings_fact* tables, but also added a HAVING clause on the aggregated data to filter out any jobs with fewer than 100 postings. This was just an arbitrary number, but considering the total number of postings fulfilling the remote Data Analyst requirements, this represented the vast majority (~ 96 %) of the job postings, with only  the niche skills removed.
...
From the above table, Python and R are close competitors for the top earning spot. Consulting my AI asisstant revealed that Python is more widely applied, supported by the number of available jobs citing this skill in the dataset. It is also better for big data and machine learning applications. R, however, is more specialized and frequently used in academia or research for statistical analysis and visualization. For now, I have decided to prioritize Python due to its wider applicability, however I will also look into R in future as I am interested in statistical evaluation of data and have been doing that informally in my research to date, although not using specific coding languages, yet. Following the programming skills are Tableau and Power BI for data visualization. While Tableau leads in the number of job postings, Power BI is less expensive and not as specialized. For this reason, as well as the fact that I have some basic experience with Power BI, I plan to start expanding my experience with Power BI first, with the idea that I could expand this learning to some specialized features in Tableau at a later stage if needed.

# Conclusions
This was a fun project to learn about the data jobs available in the job market. The databases and instructions (thanks Luke! https://www.lukebarousse.com/) were very useful to start developing an understanding of what the different positions entail, earn and require in terms of skill set. This allowed me to tailor my queries to answer questions relating to my particular topics of interest. I was suprised to see a relatively large number of specific skills required (120-160) for the various types of data jobs especially considering these weren't even the senior or specialized roles. This, along with multiple options for similar types of skills, indicates some industry segmentation and less standardization. As a newcomer to this field, the overwhelming number of technical skills and jargon can be quite intimidating. Resisting the temptation to get caught in the weeds of the technical jargon, I instead decided to apply some scientific reasoning and minimize the noise. This enabled a more cohesive story and solidified important skills to focus on for my Data Analysis journey. I confirmed that learning a skill like SQL is very useful for this kind of position, which I will further append with data visualization skills (Power BI) and programming language (Python). Since these skills are also required for Data Science, that will be a good baseline for me to add additional cloud skills (e.g. AWS) and complemenet my programming experience with something like R for specialized statistical evaluation in the longer term.
 

## Closing thoughts
The data industry is very dynamic, especially with the rapid development of AI capabilities, so the challenge remains how to best work with the technological resources available to optimize analysis time and quality of the output. I found AI assistants like ChatGPT and Claude very useful for learning about this industry and even for small and unexpected issues e.g. markdown formatting. Considering the output I got from the assistants in terms of the data analysis, I observed some gaps, where I would analyze a particular set of data differently. Rather than trying to explain and iterate what I wanted to the assistant, it was faster and easier to draw a few quick graphs in Excel and at least I know the assumptions I applied in analyzing the data. For this particular project, the database was relatively dated (from 2023), so an additional challenge would be to get more recent job postings data and determine whether the same trends hold, or whether some recent shifts have occurred. To do this, I would have to build my own databases, which would require a whole new skill set, perhaps a topic for a future challenge.
